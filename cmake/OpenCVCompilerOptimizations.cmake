...
elseif(PPC64LE)
  ...
  ocv_update(CPU_VSX_RUNTIME_TESTS "ALIGNED")
  ocv_update(CPU_VSX_ALIGNED_TEST_FILE "${OpenCV_SOURCE_DIR}/cmake/checks/runtime/cpu_vsx_aligned.cpp")
...
macro(ocv_check_compiler_optimization_runtime OPT)
  if(NOT DEFINED CPU_${OPT}_RUNTIME_SUPPORTED)
    if(CPU_${OPT}_SUPPORTED AND CPU_${OPT}_RUNTIME_TESTS)
      if(__is_from_baseline)
        if(NOT "x${CPU_${OPT}_FLAGS_ON}" STREQUAL "x")
          set(_compile_flags "${CPU_${OPT}_FLAGS_ON}")
        else()
          set(_compile_flags "${CPU_BASELINE_FLAGS}")
          ocv_append_optimization_flag(_compile_flags ${OPT})
        endif()
        foreach(_testname ${CPU_${OPT}_RUNTIME_TESTS})
          set(_var_runname "HAVE_CPU_${OPT}_RUNTIME_${_testname}")
          if(CPU_${OPT}_${_testname}_TEST_FILE)
            ocv_check_runtime_flag("${_compile_flags}" "${_var_runname}" "${CPU_${OPT}_${_testname}_TEST_FILE}")
          endif()
          if(${_var_runname})
            list(APPEND CPU_${OPT}_RUNTIME_SUPPORTED "${_testname}")
          endif()
        endforeach()
      else()
        # currently we only supports baseline so we pass tests without check in runtime for now
        foreach(_testname ${CPU_${OPT}_RUNTIME_TESTS})
          list(APPEND CPU_${OPT}_RUNTIME_SUPPORTED "${_testname}")
        endforeach()
      endif()
...
foreach(OPT ${CPU_KNOWN_OPTIMIZATIONS})
  ...
  if(__is_from_dispatch OR __is_from_baseline OR CPU_BASELINE_DETECT)
    ...
    ocv_check_compiler_optimization_runtime(${OPT})
  endif()
  ...
  if(CPU_${OPT}_RUNTIME_SUPPORTED)
    foreach(OPT_RUNTIME ${CPU_${OPT}_RUNTIME_SUPPORTED})
      list(APPEND CPU_RUNTIME_TESTS "${OPT}_${OPT_RUNTIME}")
    endforeach()
  endif()
endforeach()
...
\n\n#define CV_CPU_DISPATCH_FEATURES 0 \\")
  ...
  foreach(OPT_RUNTIME ${CPU_RUNTIME_TESTS})
    set(OPENCV_CPU_RUNTIME_DEFINITIONS_CONFIGMAKE "${OPENCV_CPU_RUNTIME_DEFINITIONS_CONFIGMAKE}
#define CV_CPU_RUNTIME_${OPT_RUNTIME} 1")
  endforeach()
  set(OPENCV_CPU_RUNTIME_DEFINITIONS_CONFIGMAKE "${OPENCV_CPU_RUNTIME_DEFINITIONS_CONFIGMAKE}\n")
